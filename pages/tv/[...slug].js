...
const TVSlug = ({ comingData, notFound, error }) => {
    ...
    <>
      ...
      <div>
        ...
        <NavigatePages
          ...
          nextPageUrl={`/tv/${genre}/${+page + 1}`}
          prevPageUrl={`/tv/${genre}/${+page - 1}`}
        />
      </div>
    </>
  );
...
export async function getServerSideProps(context) {
  ...
  if (genre === "trending") {
    ...
    endPointFirstParameter = `/trending/tv/day`;
  } else {
    endPointFirstParameter = `/tv/${genre}`;
  }
  try {
